...
public class AgileQuartzLoggerController extends AgileBaseController {
    ...
    @AgileLogger(notes = "分页查询任务执行日志", type = SysOperateType.SELECT)
    ...
    @AgileLogger(notes = "查看任务执行日志详细信息", type = SysOperateType.DETAIL)
    ...
    @AgileLogger(notes = "删除任务执行日志数据", type = SysOperateType.DELETE)
    ...
    @AgileLogger(notes = "清空任务执行日志记录", type = SysOperateType.CLEAR)
