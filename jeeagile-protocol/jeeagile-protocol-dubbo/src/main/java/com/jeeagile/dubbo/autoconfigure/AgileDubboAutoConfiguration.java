...
import com.jeeagile.dubbo.processor.AgileDubboProviderBeanPostProcessor;
import com.jeeagile.dubbo.processor.AgileDubboServiceBeanPostProcessor;
 ...
 */
@Configuration
@EnableConfigurationProperties(AgileProtocolProperties.class)
@ConditionalOnProperty(prefix = AgileConstants.AGILE_PROTOCOL, name = AgileConstants.AGILE_PROTOCOL_TYPE, havingValue = "dubbo")
@AutoConfigureAfter(AgileCoreAutoConfigure.class)
@ImportResource("classpath:dubbo/dubbo-provider.xml")
public class AgileDubboAutoConfiguration {

    @Bean
    @ConditionalOnProperty(prefix = DUBBO_SCAN_PREFIX, name = BASE_PACKAGES_PROPERTY_NAME)
    public AgileDubboServiceBeanPostProcessor dubboServiceBeanPostProcessor(PropertyResolver propertyResolver) {
        ...
        return new AgileDubboServiceBeanPostProcessor(packagesToScan);
    }


    @ConditionalOnMissingBean
    @Bean(name = AgileDubboProviderBeanPostProcessor.BEAN_NAME)
    public AgileDubboProviderBeanPostProcessor dubboProviderBeanPostProcessor() {
        return new AgileDubboProviderBeanPostProcessor();
    }
