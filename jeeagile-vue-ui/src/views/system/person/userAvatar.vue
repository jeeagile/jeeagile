...
<script>
  ...
  import { uploadPersonAvatar } from '@/api/system/person'
  ...
  export default {
    ...
    data() {
      return {
        ...
        title: '修改头像',
        options: {
          img: store.getters.userAvatar, // 裁剪图片的地址
    ...
    methods: {
      ...
      beforeUpload(file) {
        if (file.type.indexOf('image/') == -1) {
          this.messageSuccess('文件格式错误，请上传图片类型,如：JPG，PNG后缀的文件。')
        } else {
      ...
      uploadImg() {
        this.$refs.cropper.getCropBlob(data => {
          ...
          formData.append('userAvatar', data)
          uploadPersonAvatar(formData).then(response => {
            this.open = false
            this.options.img = process.env.VUE_APP_BASE_API + response.data
            store.commit('user/SET_USER_AVATAR', this.options.img)
