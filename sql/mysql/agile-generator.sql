drop table if exists agile_gen_table;
drop table if exists agile_gen_table_column;
...
(
   id                   varchar(32) not null comment '生成表主键ID',
   table_name           varchar(100) not null comment '表名称',
   table_comment        varchar(200) not null comment '表描述',
   table_type           varchar(200) not null default 'crud' comment '表类（crud单表 tree树表）',
   project_name         varchar(100) not null comment '项目名称',
   template_type        varchar(100) not null comment '模版类型',
   ...
   parent_menu_id       varchar(32) not null comment '上级菜单ID',
...
alter table agile_gen_table comment '代码生成业务表';
...
(
   ...
   pk_flag              char(1) not null default '0' comment '是否主键（1是）',
   pk_type              char(1) not null default '2' comment '主键类型（1：数据库自增  2：UUID 3：自定义）',
   required_flag        char(1) not null default '0' comment '是否必填（1是）',
   insert_flag          char(1) not null default '1' comment '是否为插入字段（1是）',
   edit_flag            char(1) not null default '0' comment '是否编辑字段（1是）',
   list_flag            char(1) not null default '0' comment '是否列表字段（1是）',
   query_flag           char(1) not null default '0' comment '是否查询字段（1是）',
   query_type           varchar(100) not null default 'EQ' comment '查询方式（等于、不等于、大于、小于、范围）',
...
alter table agile_gen_table_column comment '代码生成业务表字段信息表';

alter table agile_gen_table_column add constraint fk_gen_table_column_ref_table foreign key (table_id)
      references agile_gen_table (id) on delete restrict on update restrict;
